...
public sealed class MagicNumberFileIntegrationTests
{
    ...
    [TestCase(0, 84)] // Result tests
    [TestCase(1, 14)]
    public void GenMagicNum_FileContainsNumber_ReturnsTwiceMagnitude(int choice, double expected) {
        double result = _calculator.GenMagicNum(choice, _path, _fileReader);
        Assert.That(result, Is.EqualTo(expected));
    }

    [TestCase(-1)] // Rejection tests
    [TestCase(2)]
    public void GenMagicNum_IndexOutsideFile_ThrowsArgumentOutOfRangeException(int choice) {
        Assert.That(
